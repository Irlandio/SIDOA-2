...
class Mapos_model extends CI_Model {
    ...
    function getProdutosMinimo(){
      ...
      //  $sql = "SELECT * FROM produtos WHERE estoque <= estoqueMinimo LIMIT 10"; 
         $sql = "SELECT * FROM obs_individual LIMIT 10"; 
        return $this->db->query($sql)->result();

    }
    function getuser($table,$fields){
        
        $this->db->where('conta_Usuario <',4);
    ...
    function getOsEstatisticas(){
            $data_Do = date('Y-m-01');
         $dataInicial = date('Y-m-d', strtotime("-0 month", strtotime($data_Do)));
       // $sql = "SELECT conta, COUNT(conta) as total FROM aenpfin  WHERE dataFin >= ".$dataInicial." GROUP BY conta ORDER BY conta";
        $sql = "SELECT * FROM vendas  WHERE dataDoacao >= '".$dataInicial."'  ORDER BY atendente_id";
        return $this->db->query($sql)->result();
    }
    ...
    public function getEstatisticasFinanceiro(){
        $sql = "SELECT SUM(CASE WHEN baixado = 1 AND tipo = 'receita' THEN valor END) as total_receita, 
                       ...
                       SUM(CASE WHEN baixado = 0 AND tipo = 'despesa' THEN valor END) as total_despesa_pendente FROM itens_de_vendas";
